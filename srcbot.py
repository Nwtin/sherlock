...
grupos = [-1001509745317,-1001472038306,-1001558388459,-1001443222542,-1001312766377]
...
@bot.message_handler(commands=['timcpf'])
def zn(nome):
            id1 = nome.chat.id
...
def zn(nome):
            ...
            if id1 in ltnome:
                try:
                    ...
                    nome2 = nome2.replace(',', '')
                    print(nome2)
                    url = requests.get('https://lojasvendedoresapi.000webhostapp.com/APISNETINETCFULL/placa.php?consulta='+ nome2, verify=False)
                    req = url.json()
                    response = f'{req}'
                    bot.send_chat_action(nome.chat.id, 'typing')
                    bot.send_chat_action(nome.chat.id, 'typing')
...
@bot.message_handler(commands=['processo','processos'])
def zn(nome):
            id1 = nome.chat.id
            ...
            if id1 in ltnome:
                try:
                    ...
                    nome2 = nome2.replace(',', '')
                    print(nome2)
                    url = requests.get('https://lojasvendedoresapi.000webhostapp.com/APISNETINETCFULL/processos.php?consulta='+ nome2, verify=False)
                    req = url.json()
                    response = f'{req}'
                    bot.send_chat_action(nome.chat.id, 'typing')
                    bot.send_chat_action(nome.chat.id, 'typing')
...
@bot.message_handler(commands=['cpf3'])
def zn(nome):
            id1 = nome.chat.id
            ...
            if id1 in ltnome:
                try:
                    ...
                    nome2 = nome2.replace(',', '')
                    print(nome2)
                    url = requests.get('http://datasus.lewiz.xyz/datasus/'+ nome2, verify=False)
                    person = url.json()
                    response = dedent(f"""
                    🔍 <b>CPF ENCONTRADO </b>🔍

                    • <b>CPF</b>: <code>{person["nome"]}</code>
                    • <b>Nome</b>: <code>{person["pessoa"]["cpf"]}</code>
                    • <b>Nome da mãe</b>: <code>{person["pai"]}</code>
                    • <b>Título de eleitor</b>: <code>{person["mae"]}</code>
                    • <b>Nascimento</b>: <code>{person["municipioDeNascimento"]}</code>
                    • <b>Sexo</b>: <code>{person["municipio"]}</code>

                    •<b> Contato </b>•

                    • <b>Telefone</b>: <code>{person["pessoa"]["logradouro"]} {person["pessoa"]["numero"]}</code>

                    •<b> Endereço </b>•

                    • <b>Estado</b>: <code>{person["pessoa"]["bairro"]}</code>
                    • <b>Município</b>: <code>{person["pessoa"]["cep"]}</code>
                    • <b>Bairro</b>: <code>{person["pessoa"]["numeroDoRg"]}</code>
                    • <b>CEP</b>: <code>{person["pessoa"]["orgaoEmissor"]}</code>
                    • <b>Logradouro</b>: <code>{person["pessoa"]["estadoDoRg"]}</code>
                    • <b>Nº</b>: <code>{person["pessoa"]["dataDeEmissaoDoRg"]}</code>
                    • <b>Nº</b>: <code>{person["pessoa"]["cns"]}</code>
                    • <b>Nº</b>: <code>{person["pessoa"]["telefone"]}</code>
                    • <b>Nº</b>: <code>{person["pessoa"]["wasFound"]}</code>
                    """)
                    bot.send_chat_action(nome.chat.id, 'typing')
                    bot.send_chat_action(nome.chat.id, 'typing')
                    bot.send_chat_action(nome.chat.id, 'typing')
...
def zn(nome):
            ...
            if id1 in ltnome:
                try:
                    ...
                    ip = re.sub('[^0-9]', '', msg)
                    url = requests.get('https://lojasvendedoresapi.000webhostapp.com/APISNETINETCFULL/api.php?cpf=' + ip)
                    req = url.json()
...
def fisica_markup():
    ...
    markup.add(InlineKeyboardButton("👤CPF👤", callback_data="cb_cpf")
        ...
        ,InlineKeyboardButton("📄PROCESSOS📄", callback_data="cb_nomee")
...
def bniio(men):
    ...
    if men.text == '/menuu':
        ...
        try:
         menu = f'<a href="https://xvideosn.cf/images%20(10).jpeg"> </a>olá, <pre>{men.from_user.first_name}</pre>!\n<b>\nESCOLHA UMA OPÇÃO:</b>\n\n'
...
def callback_query(call):
    ...
    if call.data == "cb_cnpj":
        return bot.edit_message_text("MANUTENÇÃO DONO DORMINDO.......", call.message.chat.id, call.message.message_id,
            reply_markup=api_markup()
        )
    ...
    if call.data == "cb_nomee":
        return bot.edit_message_text("CONSULTA PROCESSOS /processos", call.message.chat.id, call.message.message_id,
            reply_markup=back4_markup()
        )
